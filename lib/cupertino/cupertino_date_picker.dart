...
class _EnglishNumberFormatter {
  ...
  static String formatDayOfMonth(int day, [int? dayOfWeek]) {
    return day.toString();
  }
}
...
class CupertinoDatePickerWidget extends StatefulWidget {
  ...
  ///
  /// [dateOrder] determines the order of the columns inside [CupertinoDatePickerWidget]
  ...
  /// Determines the order of the columns inside [CupertinoDatePickerWidget] in
  ...
  static const List<int> weekend = <int>[DateTime.saturday, DateTime.sunday];
  ...
  static double _getColumnWidth(
    ...
    switch (columnType) {
      ...
      case _PickerColumnType.hour:
        for (int i = 0; i < 24; i++) {
          final String hour = _EnglishNumberFormatter.formatHour(i);
          longTexts.add(hour);
        }
        break;
      ...
      case _PickerColumnType.minute:
        for (int i = 0; i < 60; i++) {
          final String minute = _EnglishNumberFormatter.formatMinute(i);
      ...
      case _PickerColumnType.dayOfMonth:
        ...
        for (int i = 1; i <= 31; i++) {
          final String dayOfMonth = _EnglishNumberFormatter.formatDayOfMonth(i);
        ...
        if (showDayOfWeek) {
          for (int wd = 1; wd < DateTime.daysPerWeek; wd++) {
            final String dayOfMonth = _EnglishNumberFormatter.formatDayOfMonth(
              longestDayOfMonth,
              wd,
            );
            longTexts.add(dayOfMonth);
      ...
      case _PickerColumnType.year:
        longTexts.add(_EnglishNumberFormatter.formatYear(2018));
        break;
...
class _CupertinoDatePickerDateTimeState
    extends State<CupertinoDatePickerWidget> {
  ...
  void _handleWeekendInitialDateTime() {
    int daysToBeAdded = 0;
    if (!widget.calendarDays.contains(initialDateTime.weekday)) {
      daysToBeAdded = widget.calendarDays.first - initialDateTime.weekday;
    }
  ...
  double _getEstimatedColumnWidth(_PickerColumnType columnType) {
    estimatedColumnWidths[columnType.index] ??=
        CupertinoDatePickerWidget._getColumnWidth(
          columnType,
          localizations,
          context,
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: CupertinoPicker.builder(
        ...
        itemBuilder: (BuildContext context, int index) {
          ...
          bool isValidDate = widget.calendarDays.contains(tempDateTime.weekday);
      ...
      child: CupertinoPicker(
        ...
        children: List<Widget>.generate(24, (int index) {
          ...
          return itemPositioningBuilder(
            context,
            Text(
              _EnglishNumberFormatter.formatHour(displayHour),
      ...
      child: CupertinoPicker(
        ...
        children: List<Widget>.generate(60 ~/ widget.minuteInterval, (
          ...
          return itemPositioningBuilder(
            context,
            Text(
              _EnglishNumberFormatter.formatMinute(minute),
  ...
  Widget _buildTimeSeperatorWidget(
    double offAxisFraction,
    TransitionBuilder itemPositioningBuilder,
    Widget? selectionOverlay,
  ) {
    return CupertinoPicker(
      ...
      children: List<Widget>.generate(1, (int index) {
        return itemPositioningBuilder(
          context,
          Text(':', style: _themeTextStyle(context)),
        );
      }),
    );
  }
  ...
  void _checkOnCustomDaysDisplay() {
    ...
    if (!widget.calendarDays.contains(selectedDate.weekday)) {
      const int daysThreshold = 1;
      final DateTime targetDate = selectedDate.add(
        const Duration(days: daysThreshold),
      );
  ...
  void _checkOnHourDisplay() {
    ...
    if (widget.selectableHourPredicate?.call(selectedDate.hour) == false) {
      const int daysThreshold = 1;
      final DateTime targetDate = selectedDate.add(
        const Duration(hours: daysThreshold),
      );
    ...
    SchedulerBinding.instance.addPostFrameCallback((Duration timestamp) {
      if (fromDate.year != newDate.year ||
        ...
        _animateColumnControllerToItem(
          dateController,
          newItemIndex ?? selectedDayFromInitial,
        );
      }
...
class _CupertinoDatePickerDateState extends State<CupertinoDatePickerWidget> {
      ...
      child: CupertinoPicker(
        ...
        children: List<Widget>.generate(31, (int index) {
          ...
          return itemPositioningBuilder(
            context,
            Text(
              _EnglishNumberFormatter.formatDayOfMonth(day, dayOfWeek),
      ...
      child: CupertinoPicker.builder(
        ...
        itemBuilder: (BuildContext context, int year) {
          ...
          return itemPositioningBuilder(
            context,
            Text(
              _EnglishNumberFormatter.formatYear(year),
...
class _CupertinoDatePickerMonthYearState
    extends State<CupertinoDatePickerWidget> {
...
/// See also:
///
///  * [CupertinoDatePickerWidget], the class that implements different display modes
